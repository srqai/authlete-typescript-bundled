...
* [processForm](#processform) - Process Revocation Request
...
async function run() {
  const result = await autheleteBundled.revocation.process({
    serviceId: "<id>",
    requestBody: {
...
import { revocationProcess } from "authelete-bundled/funcs/revocationProcess.js";
...
async function run() {
  const res = await revocationProcess(autheleteBundled, {
    serviceId: "<id>",
    requestBody: {
      parameters: "VFGsNK-5sXiqterdaR7b5QbRX9VTwVCQB87jbr2_xAI&token_type_hint=access_token",
      clientId: "26478243745571",
...
async function run() {
  const result = await autheleteBundled.revocation.processForm({
    serviceId: "<id>",
    requestBody: {
      clientLocked: false,
    },
...
```typescript
import { AutheleteBundledCore } from "authelete-bundled/core.js";
import { revocationProcessForm } from "authelete-bundled/funcs/revocationProcessForm.js";
...
async function run() {
  const res = await revocationProcessForm(autheleteBundled, {
    serviceId: "<id>",
    requestBody: {
      clientLocked: false,
    },
  ...
  if (res.ok) {
    ...
    console.log("revocationProcessForm failed:", res.error);
  }
}
