...
async function run() {
  const result = await autheleteBundled.introspection.process({
    serviceId: "<id>",
    requestBody: {
...
import { introspectionProcess } from "authelete-bundled/funcs/introspectionProcess.js";
...
async function run() {
  const res = await introspectionProcess(autheleteBundled, {
    serviceId: "<id>",
    requestBody: {
      token: "VFGsNK-5sXiqterdaR7b5QbRX9VTwVCQB87jbr2_xAI",
      scopes: [
...
async function run() {
  const result = await autheleteBundled.introspection.processForm({
    serviceId: "<id>",
    requestBody: {
      clientLocked: false,
    },
...
```typescript
import { AutheleteBundledCore } from "authelete-bundled/core.js";
import { introspectionProcessForm } from "authelete-bundled/funcs/introspectionProcessForm.js";
...
async function run() {
  const res = await introspectionProcessForm(autheleteBundled, {
    serviceId: "<id>",
    requestBody: {
      clientLocked: false,
    },
  ...
  if (res.ok) {
    ...
    console.log("introspectionProcessForm failed:", res.error);
  }
}
