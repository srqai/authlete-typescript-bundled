...
async function run() {
  const result = await autheleteBundled.authorization.process({
    serviceId: "<id>",
    requestBody: {
...
import { authorizationProcess } from "authelete-bundled/funcs/authorizationProcess.js";
...
async function run() {
  const res = await authorizationProcess(autheleteBundled, {
    serviceId: "<id>",
    requestBody: {
      parameters: "response_type=code&client_id=26478243745571&redirect_uri=https%3A%2F%2Fmy-client.example.com%2Fcb1&scope=timeline.read+history.read&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256",
    },
...
async function run() {
  const result = await autheleteBundled.authorization.processForm({
    serviceId: "<id>",
    requestBody: {
      clientLocked: true,
    },
...
```typescript
import { AutheleteBundledCore } from "authelete-bundled/core.js";
import { authorizationProcessForm } from "authelete-bundled/funcs/authorizationProcessForm.js";
...
async function run() {
  const res = await authorizationProcessForm(autheleteBundled, {
    serviceId: "<id>",
    requestBody: {
      clientLocked: false,
    },
  ...
  if (res.ok) {
    ...
    console.log("authorizationProcessForm failed:", res.error);
  }
}
