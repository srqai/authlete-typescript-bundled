...
import { AutheleteBundledDefaultError } from "../models/errors/autheletebundleddefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AutheleteBundledDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | AutheleteBundledDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AutheleteBundledDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AutheleteBundledDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
